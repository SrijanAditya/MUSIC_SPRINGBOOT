...
public class SongController {
    ...
    public String searchSong(@RequestParam("title") String title, Model model) {
        ...
        model.addAttribute("foundSong", foundSong);
        if (foundSong != null) {
            System.out.println("Found song: " + foundSong.getArtist());
        } else {
    ...
    @PostMapping(value = "/upload", consumes = "multipart/form-data")
    public String createSong(@RequestPart("title") String title, 
                             @RequestPart("artist") String artist, 
                             @RequestPart("file") MultipartFile file) throws IOException {
        if(songRepository.existsSongByFileNameEquals(file.getOriginalFilename()) || 
           songRepository.existsSongByTitleEquals(title)) {
            return "error1";
        } else {
            System.out.println("Uploading the file: " + file.getOriginalFilename());
            
            // Save file to storage
            storageService.uploadSong(file);
            
            // Create and save song to database
            Song newSong = new Song();
            newSong.setArtist(artist);
            newSong.setTitle(title);
            newSong.setFileName(file.getOriginalFilename());
            ...
            System.out.println("Song saved to database with ID: " + insertedSong.getId());
            return "redirect:/";
        }
    }
